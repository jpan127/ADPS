...
void cmd_handler_service_command(const command_packet_S * const packet)
{
    switch (packet->opcode)
    {
        ...
        case packet_opcode_incr_left     : { ESP_LOGI("", "packet_opcode_incr_left    "); motor_adjust_duty(motor_wheels , motor_dir_left          , packet->command[0], duty_increment); break; }
        case packet_opcode_incr_right    : { ESP_LOGI("", "packet_opcode_incr_right   "); motor_adjust_duty(motor_wheels , motor_dir_right         , packet->command[0], duty_increment); break; }
        ...
        case packet_opcode_decr_left     : { ESP_LOGI("", "packet_opcode_decr_left    "); motor_adjust_duty(motor_wheels , motor_dir_left          , packet->command[0], duty_decrement); break; }
        case packet_opcode_decr_right    : { ESP_LOGI("", "packet_opcode_decr_right   "); motor_adjust_duty(motor_wheels , motor_dir_right         , packet->command[0], duty_decrement); break; }
